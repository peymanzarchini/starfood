...
export const ReviewForm = ({ productId }: ReviewFormProps) => {
  ...
  const { createReview, isCreating } = useProductReviews(productId);
    ...
    <div className="bg-bg-surface dark:bg-dark-bg-surface p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <button
          type="submit"
          disabled={isCreating}
          className="w-full md:w-auto px-10 py-4 bg-primary text-white rounded-2xl font-black shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-x-2 disabled:opacity-70 cursor-pointer"
        >
          {isCreating ? (
