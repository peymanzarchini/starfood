...
/**
 * Type helpers for strict sorting values
 */
...
const FoodsPage = () => {
  ...
  // Reset to first page when search or sorting criteria changes
  useEffect(() => {
    setPage(1);
      ...
      <Container>
        ...
        <div className="flex flex-col items-center text-center mb-12">
          ...
          <p className="text-text-muted mt-3 max-w-lg font-medium">
            Discover the best fast food in town, made with fresh ingredients and lots of love.
          </p>
        </div>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-2 no-scrollbar">
            ...
            {sortOptions.map((option) => (
              <button
                ...
                className={cn(
                  "px-6 py-3 rounded-full text-xs font-black uppercase tracking-widest transition-all shrink-0 border",
        ...
        {!isLoading && data && data.pagination.totalPages > 1 && (
          <div className="flex justify-center items-center gap-4 mt-16">
            <button
              ...
              className="w-12 h-12 flex items-center justify-center rounded-full bg-bg-surface dark:bg-dark-bg-surface border border-slate-200 dark:border-slate-800 text-text-main disabled:opacity-30 transition-all hover:border-primary hover:text-primary shadow-sm active:scale-90"
            >
              <ChevronLeft size={24} />
            </button>
